...
public class update_student extends AppCompatActivity {
    ...
    public void clickUpdate(View view) {
        idno = update_idno.getText().toString();
        Name = update_name.getText().toString();
        Address = update_address.getText().toString();

        if (TextUtils.isEmpty(idno) || TextUtils.isEmpty(Name) || TextUtils.isEmpty(Address)) {
            Toast.makeText(this, "Check the Empty Fields", Toast.LENGTH_LONG).show();
        }
        else {
            ContentValues contentValues = new ContentValues();
            contentValues.put(DatabaseInfo.IDNO, idno);
